...
<head>
	...
	<script type="text/javascript">
        var professionList = ${professionListJson};
        $(function() {
            var table;
            ...
            $('#dataList').datagrid({
                ...
                columns: [[
                    ...
                    {field:'professionId',title:'所属专业',width:150, sortable: true,
                        formatter:function(value,index,row){

                            for(var i=0;i<professionList.length;i++){
                                if(professionList[i].id == value){
            ...
            $("#search-btn").click(function(){
                $('#dataList').datagrid('reload',{
                    ...
                    professionId:$("#search-profession-id").combobox('getValue')
                });
            });
        });
	</script>
	...
	<form id="addForm" method="post">
		<table id="addTable" cellpadding="8">
			...
			<tr >
				<td>所属年级:</td>
				<td>
					<select id="add_professionId"  class="easyui-combobox" style="width: 200px;" name="professionId" data-options="required:true, missingMessage:'请选择所属年级'">
						<c:forEach items="${ professionList}" var="profession">
							<option value="${profession.id }">${profession.name }</option>
						</c:forEach>
					</select>
	...
	<form id="editForm" method="post">
		...
		<table id="editTable" border=0 cellpadding="8" >
			...
			<tr >
				<td>所属年级:</td>
				<td>
					<select id="edit_professionId"  class="easyui-combobox" style="width: 200px;" name="professionId" data-options="required:true, missingMessage:'请选择所属年级'">
						<c:forEach items="${ professionList}" var="profession">
							<option value="${profession.id }">${profession.name }</option>
						</c:forEach>
					</select>
